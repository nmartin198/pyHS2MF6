...
! 
! Modifications Author: Nick Martin <nick.martin@alumni.stanford.org>
! 
...
module cphUzfModule
!====================================================================
! Copy of MODFLOW 6, v.6.2.1 UzfModule that has been slightly extended 
...
!   3. Added cphuzf_rp for package read and prepare. The original
!       uzf_rp is maintained. cphuzf_rp provides for passing in of the
!       array of UZF infiltration to the model from an external process.
!       These infiltration values are then written into the time series
!       inputs for the current day.
...
!       UZF seepage to surface and UZF rejected infiltration.
!
!====================================================================
! SPECIFICATIONS:
  ! ---------------------------------------------------------------
  ...
  integer(I4B), save :: firstTime = 0

  private
  public :: cphuzf_create
  public :: cphUzfType

  type, extends(BndType) :: cphUzfType
    ! output integers
...
contains
  ...
  subroutine cphuzf_create(packobj, id, ibcnum, inunit, iout, namemodel, pakname)
! ******************************************************************************
...
! ------------------------------------------------------------------------------
    ...
    ! -- local
    type(cphUzfType), pointer :: uzfobj
! ------------------------------------------------------------------------------
    !
...
! ******************************************************************************
! uzf_readdimensions -- set dimensions specific to cphUzfType
!
...
! Puts the infiltration from HSPF into the sinf values location so that will
!  be incorporated into the following time step solution.
!
! cphuzf_rp -- set stress data
! Subroutine: (1) check if bc changes - Not needed because always change now
!             (2) assign new bc for every day
!             (3) set kinematic variables to bc values for initial
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    ! -- dummy
    class(cphUzfType), intent(inout) :: this
    integer(I4B), intent(in) :: cpinalen
    real(DP), dimension(cpinalen), intent(in) :: cpinarr
    ! -- local
    character(len=LENBOUNDNAME) :: bndName
    character(len=LINELENGTH) :: text
    character(len=LINELENGTH) :: line
...
! ------------------------------------------------------------------------------
    ...
    this%issflag = this%gwfiss
    !
    ! Only read the file if it is the first stress period
    if ( ( kper == 1 ) .AND. ( firstTime == 0 ) ) then
      ! -- get stress period data
      if (this%ionper < kper) then
        !
        ! -- get period block
      ...
      !
      ! increment our firstTime variable
...
! ******************************************************************************
! Completely new subroutine for coupling to HSPF
!
! BC override to extract surface discharges
!     Need both groundwater seepage discharge and rejected infiltration
! ******************************************************************************
!
!    SPECIFICATIONS:
! ------------------------------------------------------------------------------
    ...
    do n = 1, this%nodes
      ...
      cgwd = this%gwd(n)
      ! groundwater discharge is negative because it leaves the model
      if ( cgwd < DZERO ) then
        cgwd = -1.0 * cgwd
      end if
      ...
      curunode = this%dis%get_nodeuser(curinode)
